<template>
    <p>Slightly different than the #1</p>
    <div class="table-component">
        <div
            v-if="showFilter && filterableColumnExists"
            class="table-component__filter"
        >
            <input
...
export default {
    ...
    computed: {
        ...
        displayedRows() {
            ...
            if (
                !this.columns.filter((column) => column.isFilterable()).length
            ) {
                return this.sortedRows;
            }

            return this.sortedRows.filter((row) =>
                row.passesFilter(this.filter)
            );
        },
